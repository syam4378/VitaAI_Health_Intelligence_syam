...
function createConnection() {
  ...
  db.connect(err => {
    if (err) {
      console.error('DB connect error:', err.message, '— retrying in 5s...');
...
function createTables() {
  ...
  tables.forEach(sql => db.query(sql, err => { if (err) console.error('Table error:', err.message); }));
...
// ── PAGES ─────────────────────────────────────────────────
app.get('/',       (_req, res) => res.sendFile(path.join(__dirname, 'public', 'signup.html')));
...
app.get('/auth/verify', verifyToken, (_req, res) => res.json({ valid: true }));
  ...
  upload.single('img')(req, res, () => {
    ...
    db.query('SELECT id, img FROM profile WHERE user_id=?', [req.user.id], (err, r) => {
      ...
      if (newImg && oldImg) {
        ...
        if (fs.existsSync(op)) try { fs.unlinkSync(op); } catch (_) {}
      }
      const vals = [
...
function calcScore(c) {
  ...
  if (c.mood) { maxPts+=20; if(c.mood.includes('Great!'))score+=20; else if(c.mood.includes('Good'))score+=15; else if(c.mood.includes('Neutral'))score+=10; else score+=4; }
  if (c.energy_level) { maxPts+=10; if(c.energy_level.includes('High'))score+=10; else if(c.energy_level.includes('Medium'))score+=8; else if(c.energy_level.includes('Low')&&!c.energy_level.includes('Very'))score+=5; else score+=2; }
...
app.post('/checkin', verifyToken, async (req, res) => {
  ...
  let prof = null;
  try { const r = await q('SELECT * FROM profile WHERE user_id=?', [req.user.id]); prof = r[0] || null; } catch (_) {}
